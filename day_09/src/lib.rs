...
pub fn part_two(input: &str) -> isize {
    input.lines()
        ...
        .map(|v| next_val_front_recursive(&v))
        .sum()
}

fn next_val_front_recursive(vals: &[isize]) -> isize {
    if vals.iter().all(|v| *v == 0 ) { return 0; }

    let first_val = vals.first().unwrap();
    let next_row: Vec<isize> = vals.windows(2).map(|v| v[1]-v[0]).collect();

    first_val - next_val_front_recursive(&next_row)
}
